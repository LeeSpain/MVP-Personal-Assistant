...
export default function ChatHistoryModal({ history, onClose, onDeleteSession }: ChatHistoryModalProps) {
    ...
    return (
        <Modal open={true} title="Chat History" onClose={onClose}>
            <div className="flex h-[60vh] gap-4">
                {/* Sidebar: List of Sessions */}
                <div className="w-1/3 border-r border-slate-800 pr-2 overflow-y-auto">
                    {history.length === 0 ? (
                        <div className="text-xs text-slate-500 text-center mt-10">No history yet.</div>
                    ) : (
                            ...
                            {history.map(session => (
                                ...
                                >
                                    ...
                                    <div className="text-[10px] opacity-70 truncate">
                                        ...
                                        {session.summary ? ` · ${session.summary}` : ` · ${session.messages.length} messages`}
                                    </div>
                                </button>
                            ))}
                        </div>
                            ...
                            <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-800">
                                <div>
                                    <h3 className="text-sm font-semibold text-slate-200">
                                        Session from {new Date(selectedSession.timestamp).toLocaleString()}
                                    </h3>
                                    <p className="text-xs text-slate-500">{selectedSession.messages.length} messages</p>
                                </div>
                                <button
                                    onClick={() => {
